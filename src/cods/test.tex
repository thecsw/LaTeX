...
\caption{\label{fig:zeros}Graph of Euler-Riemann zeta function using Wolfram|Alpha Open Code and built-in math functions.}
\end{figure}
...
by using MATLAB as shown in Figure \ref{fig:zerosv}. 

\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{./zerosv.png}
\caption{\label{fig:zerosv}Euler-Riemann zeta function values for negative even integers, calculated in MATLAB.}
\end{figure}
...
service. Table \ref{tab:bernoulli} will show first 10 non-zero elements' values
of Bernoulli numbers. 

\begin{table}[htb]
\caption{\label{tab:bernoulli}First 10 Non-Zero Elements of Bernoulli Numbers.}
\centering
